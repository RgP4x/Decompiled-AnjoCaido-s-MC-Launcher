...
public class GameUpdater implements Runnable {
  ...
  private void unloadNatives(String nativePath) {
    ...
    try {
      ...
      Vector<String> libs = (Vector)field.get(getClass().getClassLoader());
  ...
  protected void downloadJars(String path) throws Exception {
    ...
    for (int j = 0; j < this.urlList.length; j++) {
      ...
      while (downloadFile) {
        ...
        inputstream.close();
        fos.close();
        if (!(urlconnection instanceof HttpURLConnection) || fileSize == fileSizes[j])
          continue; 
        if (fileSizes[j] <= 0)
          continue; 
  ...
  public boolean canPlayOffline() {
    ...
    String[] arr$;
    int len$, i$;
    for (arr$ = gameFiles, len$ = arr$.length, i$ = 0; i$ < len$; ) {
      String necessary = arr$[i$];
